...
void arc::VertexBuffer::setVertices(const std::vector<unsigned char>& vertices, int offset, int count) {
    _isDirty = true;
    ...
    {
        _vertices[i] = (int) vertices[offset + i + 0]       |
                       (int) vertices[offset + i + 1] << 8  |
                       (int) vertices[offset + i + 2] << 16 |
                       (int) vertices[offset + i + 3] << 24;
    }
